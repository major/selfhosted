...
kubectl --kubeconfig ~/.kube/k3s-config -n cert-manager get pods

# View certificates
kubectl --kubeconfig ~/.kube/k3s-config get certificates -A

# Check certificate details
kubectl --kubeconfig ~/.kube/k3s-config -n <namespace> describe certificate <name>
...
kubectl --kubeconfig ~/.kube/k3s-config get certificates -A -o wide
```
...
kubectl --kubeconfig ~/.kube/k3s-config -n cert-manager logs -l app.kubernetes.io/name=cert-manager --tail=100

# Check certificate status
kubectl --kubeconfig ~/.kube/k3s-config -n <namespace> describe certificate <name>

# Check certificate requests
kubectl --kubeconfig ~/.kube/k3s-config get certificaterequests -A

# Verify ClusterIssuer is ready
kubectl --kubeconfig ~/.kube/k3s-config get clusterissuer
```
...
kubectl --kubeconfig ~/.kube/k3s-config -n nginx-ingress get pods

# View Nginx Ingress logs
kubectl --kubeconfig ~/.kube/k3s-config -n nginx-ingress logs -l app.kubernetes.io/name=nginx-ingress -f

# Check Ingress resources
kubectl --kubeconfig ~/.kube/k3s-config get ingress -A
```
...
flux logs --kubeconfig ~/.kube/k3s-config --level=error --follow

# Check kustomization status
flux get kustomizations --kubeconfig ~/.kube/k3s-config

# Describe a failing kustomization
kubectl --kubeconfig ~/.kube/k3s-config -n flux-system describe kustomization <name>

# Force reconciliation
flux reconcile kustomization apps --kubeconfig ~/.kube/k3s-config
```
